...
typedef struct job
{
	...
	job_arg *jarg; /* bind worker thread */
} job;
...
int thpool_add_work(thpool_ *thpool_p, void (*function_p)(void *), void *arg_p)
{
	...
	newjob->jarg = (struct job_arg *)malloc(sizeof(struct job_arg));
	newjob->jarg->thread_id = -1;
	newjob->jarg->arg = arg_p;
...
static void *thread_do(struct thread *thread_p)
{
	...
	while (threads_keepalive)
	{
		...
		if (threads_keepalive)
		{
			...
			void (*func_buff)(void *);
			// void *arg_buff;
			job_arg *jarg_buff = (struct job_arg *)malloc(sizeof(struct job_arg));
			job *job_p = jobqueue_pull(&thpool_p->jobqueue);
			if (job_p)
			{
				func_buff = job_p->function;
				if (job_p->jarg->thread_id == -1)
					jarg_buff->thread_id = thread_p->id;
				jarg_buff->arg = job_p->jarg->arg;
				func_buff((void *)jarg_buff);
				free(job_p);
				free(jarg_buff);
			}
