    ...
    for (i = 1; i < nn->num_lay; i++)
    {
        for (j = 0; j < nn->lay[i].num_neu; j++)
        {
            ...
            { // Relu for Hidden Layers
                nn->lay[i].neu[j].actv = relu(nn->lay[i].neu[j].z);
            }
            else
            { // Sigmoid for Output Layer
                nn->lay[i].neu[j].actv = sigmoid(nn->lay[i].neu[j].z);
            }
        }
    }
}
...
void compute_cost(int i, neural_network *nn, data_set *ds)
{
    ...
    for (j = 0; j < nn->lay[nn->num_lay - 1].num_neu; j++)
    {
        // nn->cost[j] = mean_square_error(ds->train_set[i][j + nn->lay[0].num_neu], nn->lay[nn->num_lay - 1].neu[j].actv);
        nn->cost[j] = binary_cross_entropy(ds->train_set[i][j + nn->lay[0].num_neu], nn->lay[nn->num_lay - 1].neu[j].actv);
        tmp += nn->cost[j];
    }
    nn->full_cost += tmp;
...
void back_prop(int p, neural_network *nn, data_set *ds)
{
    ...
    // Reset hidden layer
    for (i = 1; i < nn->num_lay - 1; i++)
    {
        for (j = 0; j < nn->lay[i].num_neu; j++)
        {
            nn->lay[i].neu[j].dactv = 0.0;
        }
    }
...
// TODO Applicable only to single output
