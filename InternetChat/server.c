...
typedef struct myevent_s       
{
    ...
    user_msg um;                // 用户登陆的信息
    int log_step;               // 标记用户位于登陆的操作
                                // 0-- 未登陆 1 --- 输入账号 2 ---- 输入密码 3----- 成功登陆
                                // 4 --- 注册用户名 5 ------ 输入注册的密码 6 ------- 再次输入密码验证
}myevent_s;
...
void event_add(int epfd, myevent_s *ev)
{
    ...
    tmp.data.ptr = ev;
    tmp.events = ev->events;
    if(epoll_ctl(epfd, EPOLL_CTL_ADD, ev->fd, &tmp) == -1)
...
{
    char str[BUFSIZ-1024],str2[1024];
    myevent_s *ev = (myevent_s *) arg;
    ...
    if(ret > 0) printf("%s", str2);
    printf("%s", str);

    sprintf(ev->buf, "(%s):%s\n>>>", ev->um.usr_name, str);   // 格式化客户端发来的数据 --- 数据处理
    ev->len = strlen(ev->buf);

    // 此时服务器端接收客户端发来的数据并进行处理,然后发送给其他的在线用户,故此时事件要重新设定为写事件
    event_del(g_efd, ev);
    event_set(ev, cfd, EPOLLOUT, cb_write, ev);
    event_add(g_efd, ev);
}
...
void load_usermsg()                 
{
    FILE *fp = fopen("/media/chrono/MHD/C/InternetChat/user_msg.txt", "r");         // 打开保存已注册用户信息的文件
...
int main(int argc, char *argv[])
{
    ...
    while(1)
    {
        ...
        //     long long interval= now_time - g_events[check_active].last_active_time;
        //     char* s=(char*)malloc(32*sizeof(char));
        //     sprintf(s, "Time interval is:%lld\n", interval);
